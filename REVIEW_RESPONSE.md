...
### R3.1 — 匿名性声明的数学错误（Remark 1）

> "The external search space is C(N,|J|), not 2^N as claimed in Remark 1. For the uniform-weight majority-threshold setting, the space is C(N, N/2). N can be less than 30 that makes the search space enumerable."

**已完成的工作：**

- 代码中 `encrypt_participation()` 对每个 signer 独立加密其参与位，外部攻击者需要尝试所有可能的 signer 子集来匹配密文——搜索空间确实为 C(N, |J|)
- 这是一个**纯论文层面的错误**，代码逻辑本身是正确的

**现状分析：**

这是三个审稿人中唯一指出的**事实性数学错误**，也是 Reviewer 3 给出 borderline 而非 accept 的直接原因之一。错误的影响分析：

- 若 N=30, |J|=15（majority），C(30,15) ≈ **1.55 × 10^8**，一台普通计算机几小时内可以枚举
- 若 N=10（小型 DAO），C(10,5) = **252**，完全无匿名性
- 论文声称的 2^N（N=30 时为 10^9）比实际值高约 6 倍（log 尺度），且更重要的是**概念上就是错的**

**尚存差距：**

- Remark 1 需要从 2^N 修正为 C(N, |J|)
- 需要诚实讨论这对 **N<30 场景**（论文的主要应用场景之一）的实际影响
- 需要为不同 N 值提供具体的匿名性量化指标（bits of anonymity = log₂ C(N, |J|)）

**实现路径：**

1. **修正 Remark 1**（论文，约 0.5 页）：
   - 将搜索空间从 2^N 改为 C(N, |J|)，并给出推导
   - 添加匿名性比特数表格：

   | N  | Threshold | log₂ C(N, \|J\|) | 安全性 |
   |----|-----------|-------------------|--------|
   | 10 | 5         | 7.8 bits          | 无     |
   | 30 | 15        | 27.2 bits         | 弱     |
   | 64 | 32        | 59.7 bits         | 中等   |
   ...
   - 诚实讨论：N≥128 时匿名性足够（>120 bits），但小规模 DAO（N<30）场景匿名性不足

2. **讨论缓解方案**（论文或 future work）：
   - Decoy/padding 技术：总是加密 N 个位置，其中非活跃 signer 的密文为 0-encryptions
   - 但这会使通信成本从 O(k) 升到 O(N)，需要权衡

---

### R3.2 — 匿名性仅对外部攻击者成立

> "Anonymity is only against outsiders. Protocol participants and the combiner can know the set of participants trivially. In a contentious governance vote, this creates concrete risks."

**已完成的工作：**

- 代码中 `sign()` 方法的 `active: &[usize]` 参数在协议参与方之间是明文传递的——这是协议的结构性特征
- 当前代码没有试图隐藏参与方身份免受内部攻击者（combiner、其他 signer）

**现状分析：**

审稿人指出了一个**威胁模型边界**问题：论文宣称的匿名性是 "against external observers"，但区块链 DAO 场景中，攻击者很可能就是**协议参与方**。

具体风险（审稿人给出的精辟分析）：
- **大额持币者 withhold 攻击**：看到 quorum 已满足，选择不签名以避免被记录（在 contentious 投票中保持 "中立"）
- **合谋/叛变**：观察到 Bctx（参与方集合）后，signer 之间可以协调或背弃
- **ballot secrecy 缺失**：DAO 治理投票中，参与方身份和投票内容应该是独立的秘密，但当前协议中参与方身份对内部是透明的

**尚存差距：**

- 论文的威胁模型声明与实际协议能力之间存在 gap，需要明确定义和收缩
- 缺少匿名通信层的讨论或引用

**实现路径：**

1. **论文修改**（约 1 页，Security Model 章节）：
   - 重新定义威胁模型为三层：
     - **Level 1 (External Observer)**：仅能看到链上数据 → 匿名性由组合搜索空间保证（R3.1 修正后）
     - **Level 2 (Passive Combiner)**：诚实但好奇的 combiner 能看到 Bctx → 匿名性无法保证
     - **Level 3 (Active Insider)**：恶意 signer 或 combiner → 无匿名性
   - 诚实说明方案仅提供 Level 1 匿名性

2. **讨论缓解方案**（可标记为 future work）：
   - **匿名通信层**：signer 通过 mixnet/Tor 发送 nonce 和部分签名，隐藏 IP 与身份的关联
   - **Private set intersection (PSI)**：使 signer 在不泄露身份集合的情况下验证 quorum
...
> "Table 1 omits a signer anonymity row, which conceals the scheme's limitations relative to competing approaches."

**已完成的工作：**

- 代码分析确认了方案的匿名性特征可以与竞品对比

**现状分析：**

这是最低成本的修改（纯论文层面），但影响很大。Table 1 是读者（和审稿人）快速评估贡献的第一入口，缺少匿名性行会让人怀疑作者在刻意隐藏方案的弱点。R3.1 和 R3.2 的匿名性问题已经暴露后，在 Table 1 中诚实标注各方案的匿名性范围更为重要。

**尚存差距：**

- Table 1 当前只比较了 Weighted、Accountability、Pairing-Free、Proof Size
- 缺少 Signer Anonymity 维度

**实现路径：**

1. **在 Table 1 中增加一行 "Signer Anonymity"**，建议内容如下：

| Scheme | Signer Anonymity |
|--------|-----------------|
| [FROST-based] | None (Bctx visible to combiner) |
| [Pairing-based WTAS] | External only (trusted tracer) |
| **This work (WTAS)** | **External only, search space C(N, \|J\|)** |
| [Ideal] | Full (internal + external) |

2. 在论文正文中简要讨论这一行揭示的 trade-off
...
| 优先级 | 意见 | 修改类型 | 预计时间 | 修改难度 |
|--------|------|---------|---------|---------|
| 🔴 P0 | R3.3 Theorem 2 → Knowledge Soundness | 论文重写 + 新证明 | 2-4 周 | 高 |
| 🔴 P0 | R3.4 Theorem 3 ZK 模拟器修复 | 论文重写 + 可能调整协议 | 1-2 周 | 高 |
| 🔴 P0 | R3.1 Remark 1 修正 | 论文局部修改 | 1 天 | 低 |
| 🟠 P1 | R2.2 Tracer 信任假设分析 | 论文新增章节 | 3-5 天 | 中 |
| 🟠 P1 | R3.2 威胁模型界定 | 论文 Security Model 修改 | 2-3 天 | 中 |
| 🟠 P1 | R3.5 权重变更讨论 | 论文新增小节 + 代码已完成 | 1 天 | 低 |
| 🟡 P2 | R2.3 Gatekeeper 讨论 | 论文修改 | 1-2 天 | 低 |
| 🟡 P2 | R1.4 验签 trade-off 讨论 | 论文修改 + 已有数据 | 1 天 | 低 |
| 🟢 P3 | R3.6 Table 1 加匿名性行 | 论文修改 | 0.5 天 | 低 |
| 🟢 P3 | R1.4 benchmark 增加加权指标 | 代码 + 论文 | 2 小时 | 低 |

**代码层面全部完成，0 项待处理。论文层面 10 项，其中 3 项（P0）为核心安全证明，必须完成才能通过下一轮审稿。**
